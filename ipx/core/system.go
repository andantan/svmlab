...
const (
	...
	//
	// The layout is fixed, so unlike most program state this is a constant
	// rather than something read off a live account:
	//
	...
	NonceAccountSpace uint64 = 80

	// NonceVersionLegacy and NonceVersionCurrent are the two layouts the
	// account has had, each a bincode variant index rather than a tag the data
	// describes itself with.
	NonceVersionLegacy  uint32 = 0
	NonceVersionCurrent uint32 = 1

	// NonceStateUninitialized and NonceStateInitialized say whether the fields
	// above hold anything, since an account is created at its full size and
	// reads as zeros until something writes it.
