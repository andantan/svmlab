...
func (h *SystemTransactionHandler) SystemNonceCreate(w http.ResponseWriter, r *http.Request) {
	...
	rentExempt, err := chain.Cli.MinimumBalanceForRentExemption(r.Context(), core.NonceAccountSpace, rpc.CommitmentConfirmed)
	if err != nil {
		handler.WriteError(w, http.StatusBadGateway, fmt.Sprintf("failed to read rent-exemption minimum: %s", err))
		return
	}
	...
	// Creating an account that already exists fails, and unlike a transfer to
	// an unfunded address there is no reading of it as intent.
	if accounts[req.NewNonceAccountKey().Base58()].Exists() {
	...
	create, err := core.System.CreateAccount(req.RentPayerKey(), req.NewNonceAccountKey(), core.System.ID(), rentExempt, core.NonceAccountSpace)
	if err != nil {
		handler.WriteError(w, http.StatusBadRequest, err.Error())
		return
	}
	...
	// above and expires with it. With one it is built against the value that
	// account stores and never expires, so which constructor runs is the
	// whole difference between the two. The nonce being borrowed here is a
	// different account from the one being created, which is what the
	// request refuses to let coincide.
	var (
		message        *types.Message
		priced         *types.Message
		nonceAuthority *types.PublicKey
	...
	// This is the floor an ordinary wallet must hold, a different number from
	// rentExempt: it is what rent_payer and fee_payer must each keep above
	// zero in their own accounts, not what NewNonceAccount needs.
	minRent, err := chain.Cli.MinimumBalanceForRentExemptionSystem(r.Context())
	if err != nil {
		handler.WriteError(w, http.StatusBadGateway, fmt.Sprintf("failed to read rent-exemption minimum: %s", err))
		return
...
// @Accept       json
// @Produce      json
// @Param        body  body      SystemNonceInitializeRequest  true  "Nonce account, authority, and fee payer"
// @Param        X-Chain-Name     header    string  true  "Chain name, e.g. solana"
// @Param        X-Chain-Network  header    string  true  "Chain network, e.g. testnet"
...
func (h *SystemTransactionHandler) SystemNonceInitialize(w http.ResponseWriter, r *http.Request) {
	...
	// A nonce account has to already hold this much to stay rent exempt once
	// initialized, resolved before the batch since it decides nothing about
	// which accounts to fetch but is needed for the balance check below.
	...
	accountInfo := accounts[req.NonceAccountKey().Base58()]
	if !accountInfo.Exists() {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("nonce_account: %s does not exist, so create it with nonce/create-account instead", req.NonceAccountKey()))
		return
	}
	if accountInfo.Owner != core.System.ID().Base58() {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("nonce_account: %s is owned by %s, and a nonce account is owned by the System Program", req.NonceAccountKey(), accountInfo.Owner))
		return
	}
	if accountInfo.Space != core.NonceAccountSpace {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("nonce_account: %s is %d bytes, and a nonce account is %d", req.NonceAccountKey(), accountInfo.Space, core.NonceAccountSpace))
		return
	}

	data, err := accountInfo.Bytes()
	if err != nil {
		handler.WriteError(w, http.StatusBadGateway, fmt.Sprintf("failed to decode account data: %s", err))
		return
	}
	existingNonce, err := core.DeserializeNonceAccount(data)
	if err != nil {
		handler.WriteError(w, http.StatusBadRequest, err.Error())
		return
	}
	if existingNonce.Initialized() {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("nonce_account: %s is already initialized, with %s as its authority", req.NonceAccountKey(), existingNonce.Authority))
		return
	}
	if accountInfo.Lamports < nonceRent {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("nonce_account: %s holds %d lamports, below the %d lamport rent-exemption minimum for %d bytes", req.NonceAccountKey(), accountInfo.Lamports, nonceRent, core.NonceAccountSpace))
		return
	}
	...
	// above and expires with it. With one it is built against the value that
	// account stores and never expires, so which constructor runs is the
	// whole difference between the two. Both accounts here are nonce
	// accounts, so the names say which is which: buildNonce is the one being
	// borrowed, and the one being initialized was read above.
	var (
		message        *types.Message
		priced         *types.Message
		nonceAuthority *types.PublicKey
	...
	if req.DurableNonceAccountKey().IsNil() {
		...
		buildNonce, err := accounts[req.DurableNonceAccountKey().Base58()].NonceAccount()
		if err != nil {
			handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("durable_nonce_account: %s %s", req.DurableNonceAccountKey(), err))
			return
		}

		advance, err := core.System.AdvanceNonceAccount(req.DurableNonceAccountKey(), buildNonce.Authority)
		if err != nil {
			handler.WriteError(w, http.StatusBadRequest, err.Error())
			return
		}
	...
	// This is the floor an ordinary wallet must hold: what fee_payer must
	// keep above zero in its own account. Initialize moves no lamports of
	// its own, so fee_payer is the only role checked here.
	minRent, err := chain.Cli.MinimumBalanceForRentExemptionSystem(r.Context())
	if err != nil {
		handler.WriteError(w, http.StatusBadGateway, fmt.Sprintf("failed to read rent-exemption minimum: %s", err))
		return
	...
	// nonceAuthority is nil without a nonce, and IsNil is nil safe.
	handler.WriteOK(w, NewSystemNonceInitializeResponse(tx, raw, messageBytes, req.NonceAccountKey(), req.AuthorityKey(), req.FeePayerKey(), nonceAuthority, fee))
}
...
// @Accept       json
// @Produce      json
// @Param        body  body      SystemNonceAdvanceRequest  true  "Nonce account, authority, and fee payer"
// @Param        X-Chain-Name     header    string  true  "Chain name, e.g. solana"
// @Param        X-Chain-Network  header    string  true  "Chain network, e.g. testnet"
...
func (h *SystemTransactionHandler) SystemNonceAdvance(w http.ResponseWriter, r *http.Request) {
	...
	// One lookup covers everything the instruction will check: the account
	// exists, the System Program owns it, it is the right size, it is
	// initialized, and authority is really its authority.
	accountInfo := accounts[req.NonceAccountKey().Base58()]
	if !accountInfo.Exists() {
	...
	if !nonce.Authority.Equal(req.AuthorityKey()) {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("authority: %s is not the authority of %s, which is %s", req.AuthorityKey(), req.NonceAccountKey(), nonce.Authority))
		return
	}
	...
	// account stores and never expires, so which constructor runs is the
	// whole difference between the two. Both accounts here are nonce
	// accounts and both get advanced, which is exactly why the request
	// refuses to let them coincide: a transaction may advance a nonce only
	// once.
	var (
		message        *types.Message
		priced         *types.Message
	...
	// This is the floor an ordinary wallet must hold: what fee_payer must
	// keep above zero in its own account. Advance moves no lamports of its
	// own, so fee_payer is the only role checked here.
	minRent, err := chain.Cli.MinimumBalanceForRentExemptionSystem(r.Context())
	if err != nil {
		handler.WriteError(w, http.StatusBadGateway, fmt.Sprintf("failed to read rent-exemption minimum: %s", err))
	...
	// nonceAuthority is nil without a nonce, and IsNil is nil safe.
	handler.WriteOK(w, NewSystemNonceAdvanceResponse(tx, raw, messageBytes, req.NonceAccountKey(), req.AuthorityKey(), req.FeePayerKey(), nonceAuthority, nonce.Nonce, fee))
}
...
// @Accept       json
// @Produce      json
// @Param        body  body      SystemNonceWithdrawRequest  true  "Nonce account, authority, recipient, lamports, and fee payer"
// @Param        X-Chain-Name     header    string  true  "Chain name, e.g. solana"
// @Param        X-Chain-Network  header    string  true  "Chain network, e.g. testnet"
...
func (h *SystemTransactionHandler) SystemNonceWithdraw(w http.ResponseWriter, r *http.Request) {
	...
	// exists, the System Program owns it, it is the right size, it is
	// initialized, and authority is really its authority.
	nonceInfo := accounts[req.NonceAccountKey().Base58()]
	if !nonceInfo.Exists() {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("nonce_account: %s does not exist", req.NonceAccountKey()))
		return
	}
	if nonceInfo.Owner != core.System.ID().Base58() {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("nonce_account: %s is owned by %s, and a nonce account is owned by the System Program", req.NonceAccountKey(), nonceInfo.Owner))
		return
	}
	if nonceInfo.Space != core.NonceAccountSpace {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("nonce_account: %s is %d bytes, and a nonce account is %d", req.NonceAccountKey(), nonceInfo.Space, core.NonceAccountSpace))
		return
	}
	...
	if nonceInfo.Lamports < req.ToLamports() {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("lamports: %s holds %d lamports, short of %d", req.NonceAccountKey(), nonceInfo.Lamports, req.ToLamports()))
		return
	}

	remaining := nonceInfo.Lamports - req.ToLamports()
	if remaining == 0 {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("lamports: taking all %d lamports closes the account, which nonce/withdraw/max does", nonceInfo.Lamports))
		return
	}
	if remaining < nonceRent {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("lamports: would leave %s with %d lamports, below the %d lamport rent-exemption minimum for %d bytes", req.NonceAccountKey(), remaining, nonceRent, core.NonceAccountSpace))
		return
	}

	ix, err := core.System.WithdrawNonceAccount(req.NonceAccountKey(), req.AuthorityKey(), req.RecipientAccountKey(), req.ToLamports())
	if err != nil {
		handler.WriteError(w, http.StatusBadRequest, err.Error())
		return
	}
	...
	// whole difference between the two. Both accounts here are nonce
	// accounts, so the names say which is which: buildNonce is the one being
	// borrowed, and the one being withdrawn from was read above.
	var (
		message        *types.Message
		priced         *types.Message
		nonceAuthority *types.PublicKey
	...
	// This is the floor an ordinary wallet must hold: what fee_payer must
	// keep above zero in its own account.
	minRent, err := chain.Cli.MinimumBalanceForRentExemptionSystem(r.Context())
	if err != nil {
		handler.WriteError(w, http.StatusBadGateway, fmt.Sprintf("failed to read rent-exemption minimum: %s", err))
		return
	...
	// nonceAuthority is nil without a nonce, and IsNil is nil safe.
	handler.WriteOK(w, NewSystemNonceWithdrawResponse(tx, raw, messageBytes, req.NonceAccountKey(), req.AuthorityKey(), req.FeePayerKey(), nonceAuthority, req.ToLamports(), remaining, fee))
}
...
// @Accept       json
// @Produce      json
// @Param        body  body      SystemNonceWithdrawMaxRequest  true  "Nonce account, authority, recipient, and fee payer"
// @Param        X-Chain-Name     header    string  true  "Chain name, e.g. solana"
// @Param        X-Chain-Network  header    string  true  "Chain network, e.g. testnet"
...
func (h *SystemTransactionHandler) SystemNonceWithdrawMax(w http.ResponseWriter, r *http.Request) {
	...
	lamports := nonceInfo.Lamports

	ix, err := core.System.WithdrawNonceAccount(req.NonceAccountKey(), req.AuthorityKey(), req.RecipientAccountKey(), lamports)
	if err != nil {
		handler.WriteError(w, http.StatusBadRequest, err.Error())
		return
	}
	...
	// whole difference between the two. Both accounts here are nonce
	// accounts, so the names say which is which: buildNonce is the one being
	// borrowed, and the one being closed was read above.
	//
	// Unlike the other max endpoints this needs no probe. The amount is
	// nonce_account's whole balance with nothing subtracted, since the fee
	// payer cannot be nonce_account, so it does not depend on the fee and
	// the real message can be priced directly.
	var (
		message        *types.Message
		priced         *types.Message
		nonceAuthority *types.PublicKey
	...
	// nonceAuthority is nil without a nonce, and IsNil is nil safe.
	handler.WriteOK(w, NewSystemNonceWithdrawMaxResponse(tx, raw, messageBytes, req.NonceAccountKey(), req.AuthorityKey(), req.FeePayerKey(), nonceAuthority, lamports, fee))
}
...
// @Accept       json
// @Produce      json
// @Param        body  body      SystemNonceAuthorizeRequest  true  "Nonce account, current authority, new authority, and fee payer"
// @Param        X-Chain-Name     header    string  true  "Chain name, e.g. solana"
// @Param        X-Chain-Network  header    string  true  "Chain network, e.g. testnet"
...
func (h *SystemTransactionHandler) SystemNonceAuthorize(w http.ResponseWriter, r *http.Request) {
	...
	// whole difference between the two. Both accounts here are nonce
	// accounts, so the names say which is which: buildNonce is the one being
	// borrowed, and the one changing hands was read above.
	var (
		message        *types.Message
		priced         *types.Message
		nonceAuthority *types.PublicKey
	...
	// This is the floor an ordinary wallet must hold: what fee_payer must
	// keep above zero in its own account. Authorize moves no lamports of its
	// own, so fee_payer is the only role checked here.
	minRent, err := chain.Cli.MinimumBalanceForRentExemptionSystem(r.Context())
	if err != nil {
		handler.WriteError(w, http.StatusBadGateway, fmt.Sprintf("failed to read rent-exemption minimum: %s", err))
	...
	// nonceAuthority is nil without a nonce, and IsNil is nil safe.
	handler.WriteOK(w, NewSystemNonceAuthorizeResponse(tx, raw, messageBytes, req.NonceAccountKey(), req.AuthorityKey(), req.NewAuthorityKey(), req.FeePayerKey(), nonceAuthority, fee))
}
...
// @Accept       json
// @Produce      json
// @Param        body  body      SystemNonceUpgradeRequest  true  "Nonce account and fee payer"
// @Param        X-Chain-Name     header    string  true  "Chain name, e.g. solana"
// @Param        X-Chain-Network  header    string  true  "Chain network, e.g. testnet"
...
func (h *SystemTransactionHandler) SystemNonceUpgrade(w http.ResponseWriter, r *http.Request) {
	...
	// One lookup covers everything the instruction will check: the account
	// exists, the System Program owns it, it is the right size, it is
	// initialized, and it is still Legacy.
	nonceInfo := accounts[req.NonceAccountKey().Base58()]
	if !nonceInfo.Exists() {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("nonce_account: %s does not exist", req.NonceAccountKey()))
		return
	...
	if nonce.Version != core.NonceVersionLegacy {
		handler.WriteError(w, http.StatusBadRequest, fmt.Sprintf("nonce_account: %s is already at version %d, and only a Legacy account can be upgraded", req.NonceAccountKey(), nonce.Version))
		return
	}
	...
	// whole difference between the two. Both accounts here are nonce
	// accounts, so the names say which is which: buildNonce is the one being
	// borrowed, and the Legacy one being migrated was read above.
	var (
		message        *types.Message
		priced         *types.Message
		nonceAuthority *types.PublicKey
	...
	// This is the floor an ordinary wallet must hold: what fee_payer must
	// keep above zero in its own account. Upgrade moves no lamports of its
	// own, so fee_payer is the only role checked here.
	minRent, err := chain.Cli.MinimumBalanceForRentExemptionSystem(r.Context())
	if err != nil {
		handler.WriteError(w, http.StatusBadGateway, fmt.Sprintf("failed to read rent-exemption minimum: %s", err))
	...
	// nonceAuthority is nil without a nonce, and IsNil is nil safe.
	handler.WriteOK(w, NewSystemNonceUpgradeResponse(tx, raw, messageBytes, req.NonceAccountKey(), req.FeePayerKey(), nonceAuthority, nonce.Version, fee))
}
