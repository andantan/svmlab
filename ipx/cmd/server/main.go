...
func run() error {
	...
	r.Route("/svm/tool", func(r chi.Router) {
		...
		r.Post("/prove/confidential-transfer-with-fee", tool.ProveConfidentialTransferWithFee)
		...
		r.Post("/prove/confidential-mint", tool.ProveConfidentialMint)
		r.Post("/prove/confidential-burn", tool.ProveConfidentialBurn)
		r.Post("/prove/confidential-rotate-supply-elgamal-pubkey", tool.ProveConfidentialRotateSupplyElGamalPubkey)
		r.Post("/prove/confidential-withdraw-withheld-from-mint", tool.ProveConfidentialWithdrawWithheldFromMint)
		r.Post("/prove/confidential-withdraw-withheld-from-accounts", tool.ProveConfidentialWithdrawWithheldFromAccounts)
		r.Post("/split/record-chunks", tool.SplitRecordChunks)
	...
	r.Route("/svm/v2", func(r chi.Router) {
		...
		r.Route("/transaction/token", func(r chi.Router) {
			...
			r.Route("/extensions/confidential-mint-burn", func(r chi.Router) {
				...
				r.Post("/mint", tk.ConfidentialMint)
				r.Post("/burn", tk.ConfidentialBurn)
				r.Post("/apply-pending-burn", tk.ConfidentialApplyPendingBurn)
				r.Post("/rotate-supply-elgamal-pubkey", tk.ConfidentialRotateSupplyElGamalPubkey)
			...
			r.Route("/extensions/confidential-transfer-fee-config", func(r chi.Router) {
				...
				r.Post("/harvest-withheld-tokens-to-mint", tk.ConfidentialHarvestWithheldTokensToMint)
				r.Post("/withdraw-withheld-tokens-from-mint", tk.ConfidentialWithdrawWithheldTokensFromMint)
				r.Post("/withdraw-withheld-tokens-from-accounts", tk.ConfidentialWithdrawWithheldTokensFromAccounts)
			})
		...
		rec := v2.NewRecordTransactionHandler(cfg)
		r.Route("/transaction/record", func(r chi.Router) {
			r.Post("/create-account", rec.RecordCreateAccount)
			r.Post("/initialize", rec.RecordInitialize)
			r.Post("/write", rec.RecordWrite)
		...
		r.Route("/transaction/zk-elgamal-proof", func(r chi.Router) {
			r.Route("/context-state", func(r chi.Router) {
				...
				r.Post("/verify-from-account/pubkey-validity", zk.ContextStateVerifyFromAccountPubkeyValidity)
				r.Post("/verify-from-account/ciphertext-commitment-equality", zk.ContextStateVerifyFromAccountCiphertextCommitmentEquality)
				r.Post("/verify-from-account/batched-grouped-ciphertext-3-handles-validity", zk.ContextStateVerifyFromAccountBatchedGroupedCiphertext3HandlesValidity)
				r.Post("/verify-from-account/batched-range-proof-u128", zk.ContextStateVerifyFromAccountBatchedRangeProofU128)
				r.Post("/verify-from-account/zero-ciphertext", zk.ContextStateVerifyFromAccountZeroCiphertext)
				r.Post("/verify-from-account/ciphertext-ciphertext-equality", zk.ContextStateVerifyFromAccountCiphertextCiphertextEquality)
				r.Post("/verify-from-account/percentage-with-cap", zk.ContextStateVerifyFromAccountPercentageWithCap)
				r.Post("/verify-from-account/batched-range-proof-u64", zk.ContextStateVerifyFromAccountBatchedRangeProofU64)
				r.Post("/verify-from-account/batched-range-proof-u256", zk.ContextStateVerifyFromAccountBatchedRangeProofU256)
				r.Post("/verify-from-account/grouped-ciphertext-2-handles-validity", zk.ContextStateVerifyFromAccountGroupedCiphertext2HandlesValidity)
				r.Post("/verify-from-account/batched-grouped-ciphertext-2-handles-validity", zk.ContextStateVerifyFromAccountBatchedGroupedCiphertext2HandlesValidity)
				r.Post("/verify-from-account/grouped-ciphertext-3-handles-validity", zk.ContextStateVerifyFromAccountGroupedCiphertext3HandlesValidity)
